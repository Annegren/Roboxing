...
class OrderManager extends AbstractManager
{
    ...
    {
        $query = 'SELECT robot.id,robot.name,image,accessoire,combat.id,lieu,date,type,user.id,username FROM ' . self::TABLE . ' JOIN combat ON combat.id=order.combat_id JOIN robot ON robot.id=order.robot_id JOIN user ON user.id=order.user_id';
        if ($orderBy) {
